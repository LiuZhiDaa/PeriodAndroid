...
public class OutSceneCool implements IOutScene {
    @Override
    ...
    public boolean isNeedTriggered(JSONObject condition) {
        ...
        String strSceneTime = sp.getString("out_cool_scene_date", "");
        String strTime = UtilsTime.getDateStringYyyyMmDd(lTime);
        if (0 != strTime.compareTo(strSceneTime)) {
            sp.edit().putString("out_cool_scene_date", strTime).apply();
            sp.edit().putInt("out_cool_scene_count", 0).apply();
        }

        // 场景每天次数限制
        int nSceneCount = sp.getInt("out_cool_scene_count", 0);
        if (nSceneCount >= iOutConfig.getOutSceneCountLimitOneDay(getType()))
            return false;

        // 保护时间
        long lSceneTriggeredTime = sp.getLong("out_cool_scene_triggered_time", 0);
        ...
        // 触发逻辑判断
        float fTemperature = UtilsSystem.getBatteryTemperatureValue(XOutFactory.getApplication());
        IOutSceneConfig outSceneConfig = iOutConfig.getOutSceneConfig(getType());
        if (outSceneConfig == null) {
            return false;
        }
        if (fTemperature < outSceneConfig.getOutSceneCoolTemperature())//38
            return false;
    ...
    public boolean updateTriggered() {
        SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(XOutFactory.getApplication());
        int nSceneCount = sp.getInt("out_cool_scene_count", 0);
        sp.edit().putInt("out_cool_scene_count", ++nSceneCount).apply();
        sp.edit().putLong("out_cool_scene_triggered_time", System.currentTimeMillis()).apply();
        return true;
    }
    ...
    public boolean executeAsync(final IOutSceneListener iOutSceneListener) {
        ...
        iXThreadPool.addTask(new IXThreadPoolListener() {
            ...
            public void onTaskComplete() {
                if (null != iOutSceneListener)
                    iOutSceneListener.onExecuteAsyncComplete(OutSceneCool.this);
            }
