...
public class OutSceneNetwork implements IOutScene {
    @Override
    ...
    public boolean isNeedTriggered(JSONObject condition) {
        ...
        String strSceneTime = sp.getString("out_network_scene_date", "");
        String strTime = UtilsTime.getDateStringYyyyMmDd(lTime);
        if (0 != strTime.compareTo(strSceneTime)) {
            sp.edit().putString("out_network_scene_date", strTime).apply();
            sp.edit().putInt("out_network_scene_count", 0).apply();
        }

        // 场景每天次数限制
        int nSceneCount = sp.getInt("out_network_scene_count", 0);
        if (nSceneCount >= iOutConfig.getOutSceneCountLimitOneDay(getType()))
            return false;

        // 保护时间
        long lSceneTriggeredTime = sp.getLong("out_network_scene_triggered_time", 0);
    ...
    public boolean updateTriggered() {
        SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(XOutFactory.getApplication());
        int nSceneCount = sp.getInt("out_network_scene_count", 0);
        sp.edit().putInt("out_network_scene_count", ++nSceneCount).apply();
        sp.edit().putLong("out_network_scene_triggered_time", System.currentTimeMillis()).apply();
        return true;
    }
    ...
    public boolean executeAsync(final IOutSceneListener iOutSceneListener) {
        ...
        iXThreadPool.addTask(new IXThreadPoolListener() {
            ...
            public void onTaskComplete() {
                if (null != iOutSceneListener)
                    iOutSceneListener.onExecuteAsyncComplete(OutSceneNetwork.this);
            }
